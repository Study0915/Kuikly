async (page) => {
  ...
  await scene('展开事实核对：11.50 → 10.80，−6.09%\n显示端点、分母与“不是最大回撤”的边界',async()=>{await label('查看计算').click();await label('计算明细').waitFor();await label('计算明细').scrollIntoViewIfNeeded()});
  ...
  await scene('量能也能核对：600 ÷ 5 = 120万股\n180 ÷ 120 = 1.50倍',async()=>{await label('量能观察').click();await label('计算明细').waitFor();await label('计算明细').scrollIntoViewIfNeeded()});
  ...
  await scene('切换缺量：09-02缺失，量能比较不可用\n不跳过缺失日补样本，价格依据仍然有效',async()=>{await label('演示设置').click();await label('量能缺失').click();await label('当前行情事实').waitFor();await label('量能观察 · 暂不可用').scrollIntoViewIfNeeded()});
  ...
  await scene('首次失败有明确恢复动作\n重试保留同一股票，不伪造加载成功',async()=>{await label('演示设置').click();await label('首次失败').click();await label('重试当前股票').waitFor()});
