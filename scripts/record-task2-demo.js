async (page) => {
  ...
  await scene('追问携带 B 上下文，并保留未发送草稿\n先读风险结论，再核对卡片依据',async()=>{await label('股票问题输入').fill('分析 L 的风险（尚未发送）');await label('追问 B 风险').click();await label('回答 3').getByLabel('B 行情证据卡',{exact:true}).waitFor();await label('定位最新回答').click()});
  ...
  await scene('比较 C 和 Z：明确提示超出演示范围\n不会把用户对象悄悄换成 A 和 B',async()=>{await label('股票问题输入').fill('比较 C 和 Z');await label('发送问题').click();await app.waitForFunction(()=>document.querySelector('[aria-label="回答 5"]')?.innerText.includes('暂无匹配'));await label('回答 5').scrollIntoViewIfNeeded()});
  await scene('演示首次失败：原问题保留，可原位重试\n正常流程不依赖网络或真实模型',async()=>{await label('演示设置').click();await label('切换回答场景').click();await label('股票问题输入').fill('分析 B');await label('发送问题').click();await label('重试问题 6').waitFor();await label('回答 6').scrollIntoViewIfNeeded()});
