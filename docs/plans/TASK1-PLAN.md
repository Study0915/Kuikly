...
用户本轮指定以 v3 开始 Task 1 实施计划，并要求 Git commit 管理、依赖安装在工作区、避免污染 base 环境。方案方向沿用 v3，不重新要求选择创新候选；本轮授权为编写及提交计划。完整实施合同确认后，Codex 才进入 CODE。
...
**完整合同待确认：**本 v1.0 的实现顺序、文件/状态/路由、20 日点按范围、测试与平台限制，以及进入 CODE。根据 [AGENTS.md](../../AGENTS.md)“用户确认后才能进入 CODE”，计划完成后 T1-PLAN 置 WAITING_USER，T1-CODE 保持 BACKLOG。此门来自仓库流程，不是工具或技能新增审批。

下一步：用户确认本计划并授权 CODE 后，Codex 记录确认版本及实际 HEAD，从 C0 工作区隔离检查开始，再执行 C1 双图绘制、点选和滚动协调探针；不提前启动 Task 2，不推送、合并、PR、tag、发布或对外沟通。
