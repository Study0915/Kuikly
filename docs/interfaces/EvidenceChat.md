...
输入框发送成功才清空草稿；快捷问题和卡片追问保留已有草稿。“最新回答”定位最后一轮的问题与摘要，使用当前布局的真实位置。最多20轮全部先测量位置，实际DOM仍由List按可见范围创建。视图代次隔离离页回调，请求票据隔离新会话与历史重试的延迟滚动。
...
比较必须明确两只不同的A–L股票，例如“比较 C 和 D”；对象不足、重复、多于两只或包含范围外股票均返回具体提示，不补入A/B或忽略多余对象。完整ASCII token识别避免把八位日期、长数量误作六位股票代码；数量单位前的数字也不作为代码。这仍是受限示例语法，不声称通用自然语言理解。
...
围栏内解析保留原始缩进、空行和字面标记；这项由共同逻辑测试验证。当前Mock回答不生成代码围栏，不把解析单测写成原生设备排版验证。
