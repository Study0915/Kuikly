...
2026-09-12 持续优化：沿用现有 chat/UI 结构，实体识别限制为已公开的 Mock 标识；歧义比较直接返回提示，不补造实体。草稿发送与快捷追问分开处理。列表导航使用当前挂载视图的实际布局，并在离开会话时解绑。交付脚本新增工作区收据：构建输入、H5/APK 与视频均以 SHA256 绑定，防止更新源码后继续打入旧产物。此批次不新增业务依赖或 module，详见总计划 v1.1。
